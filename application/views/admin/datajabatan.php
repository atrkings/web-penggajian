...
<table class="table table-borderd table-striped mt-2">
    <tr>
        ...
        <th class="text-center">Tunjangan Kesehatan</th>
    ...
    <?php $no=1; foreach ($jabatan as $j) : ?>
        <tr>
            ...
            <td>Rp. <?= number_format($j->gaji_lembur,0,',','.') ?></td>
            <td>Rp. <?= number_format($j->tj_kesehatan,0,',','.') ?></td>
            <td>Rp. <?= number_format($j->gaji_pokok + $j->gaji_lembur + $j->tj_kesehatan ,0,',','.') ?></td>
            <td>
                <center>
                    ...
                    <a onclick="return confirm('Yakin, Hapus <?= $j->nama_jabatan ?> ?')" class="btn btn-sm btn-danger" href="<?= base_url('admin/dataJabatan/deleteData/' .$j->id_jabatan) ?>">
